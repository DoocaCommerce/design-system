...
Ao usar qualquer atributo suportado pela tag `input` do tipo `file` elas serão repassadas para o componente de forma válida "porém prevalece as props configuradas no componente". O único atributo que não será repassado para o `input` interno é o `class` que é usado no elemento principal do componente "o que contém a classe `drop-files`", permitindo assim adicionar classes personalizadas ao mesmo.
...
Também é possível usar os formatos permitidos de forma mista com extensão e MIME type. Exemplo: `['.png', 'image/jpeg']`.
...
Ao definir uma extensão como tipo permitido o componente valida se o MIME type existente no arquivo em carregamento para aquela determinada extensão é valido, se for inválido o arquivo é rejeitado. Por exemplo, para permitir imagens PNG, a prop `allowedFormats` pode ser definida como `['.png']`. Com uso de extensão garantimos os múltiplos tipos de MIME type que podem ser aceitos para aquela extensão, porém limitado ao suporte da biblioteca.

O componente suporta um número limitado de extensões de arquivos, de alguns formatos obtidos a partir de um [banco de dados de tipos MIME da comunidade](https://mimetype.io/all-types). A lista na seção [Mime Types](#mime-types) lista os formatos atualmente suportados.
